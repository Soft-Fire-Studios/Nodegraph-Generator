/*
	Credits to Silverlan for this beautiful code capable of reading and saving .ain files <3
*/
...
function methods:ParseFile(f)
	f = file.Open(f,"rb","GAME")
		...
		if(numNodes > CH_MAX_NODES or numNodes < 0) then
...
function methods:AddNode(pos,type,yaw,info)
	type = CH_NODETRANS[type] or 1
	local nodes = self:GetNodes()
	local numNodes = #nodes
	if(numNodes == CH_MAX_NODES) then return false end
	local offset = {}
...
function methods:RemoveLink(src,dest)
	...
	if(!nodeSrc or !nodeDest) then return end
	local links = self:GetLinks()
	for _,link in pairs(links) do
		if((link.src == nodeSrc && link.dest == nodeDest) or (link.src == nodeDest && link.dest == nodeSrc)) then
			links[_] = nil
		end
	end
	for _,linkSrc in pairs(nodeSrc.link) do
		if((linkSrc.src == nodeSrc && linkSrc.dest == nodeDest) or (linkSrc.src == nodeDest && linkSrc.dest == nodeSrc)) then
			nodeSrc.link[_] = nil
		end
	end
	for _,linkDest in pairs(nodeDest.link) do
		if((linkDest.src == nodeSrc && linkDest.dest == nodeDest) or (linkDest.src == nodeDest && linkDest.dest == nodeSrc)) then
			nodeDest.link[_] = nil
		end
	end
end
...
function methods:Save(f)
	if(!f) then
		...
		f = "ch_nodegraph/" .. game.GetMap() .. ".ain.txt"
	end
	local mapDir = f
